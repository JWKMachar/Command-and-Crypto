export class Defender extends Phaser.GameObjects.Image {
  ...
  update() {
    ...
    let found = window.state.bots.find(
      (x) => x.x == this.targetX && x.y == this.targetY
    );
    if (!found) {
      this.targetX = 10000;
      this.targetY = 10000;
    ...
    if (currEucl < 32 && window.state.Enemy[0] != undefined) {
      var minPos = 0;
      var minEucl = Math.sqrt(
        (this.x - window.state.Enemy[0].x) *
          (this.x - window.state.Enemy[0].x) +
          (this.y - window.state.Enemy[0].y) *
            (this.y - window.state.Enemy[0].y)
      );
      ...
      window.state.Enemy = window.state.Enemy.filter(
        (x) => x != this.workingOn
      );
      setTimeout(() => {
